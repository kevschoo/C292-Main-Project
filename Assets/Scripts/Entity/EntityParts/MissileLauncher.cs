...
[System.Serializable]
public class MissileLauncher : EntitySystem
{

    [field: SerializeField] public override string SystemName { get; set; } = "MissileLauncher";
    ...
    [SerializeField] int MaxTargetsToPenetrate = 1;
    [SerializeField] float BulletSpeed = 14;
    [SerializeField] float BulletLifetime = 3;
    [SerializeField] float BaseFireCooldown = 2;
    bool IsOnCoolDown;
    Vector3 AimPoint;
    ...
    void Start()
    {
        ...
        if (this.gameObject.TryGetComponent<EntityAI>(out EntityAI PAI))
        {
            ...
            {
                BulletType = ParentAI.OffensiveSystemData.SpawnableObjects.Where(gameObject => gameObject.name == "MissileLauncher").SingleOrDefault();
            }
            else
            {
